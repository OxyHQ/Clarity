...
test('missing static files remain 404 while navigation retains the SPA fallback', async () => {
  const env = { ASSETS: { fetch: async () => new Response('shell', { headers: { 'content-type': 'text/html' } }) } };
  const ctx = { waitUntil() {} };
  assert.equal((await worker0.fetch(new Request('https://clarity.surf/missing.js'), env, ctx)).status, 404);
  assert.equal(await (await worker0.fetch(new Request('https://clarity.surf/settings'), env, ctx)).text(), 'shell');
});
...
test('enabled edge producer publishes actual media operations without a dashboard viewer', async () => {
  ...
  try {
    ...
    assert.deepEqual(events.map(event => [event.service, event.region, event.scope, event.direction, event.activityType]), [
      ['clarity', 'edge-mad', 'external', 'inbound', 'media'],
      ['clarity', 'edge-mad', 'external', 'outbound', 'media'],
    ]);
